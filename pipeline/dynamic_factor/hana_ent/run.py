...
def prepare_dfm_single_window(X: pd.DataFrame, min_var: float = 1e-8) -> Tuple[pd.DataFrame, StandardScaler]:
    """
...
def fit_supply_models(factors: pd.DataFrame,
    ...
    for col in reg_cfg.target_columns:
        print(f"[DEBUG] Fitting model for {col}")
...
def rolling_method_dfm(
    ...
    for end_idx in ends:
        ...
        # Prepare window
        Xw, _ = prepare_dfm_single_window(Xw_raw)
